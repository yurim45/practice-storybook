...
type ButtonProps = {
	...
	type?: 'submit' | 'button';
	size?: 'small' | 'full';
	styleConcept?: 'default' | 'except'; // 디폴트, 제외
	disabled?: boolean;
	onClick?: () => void;
};
...
const Button = ({
	...
	return (
		<Inner styleConcept={styleConcept} type={type} {...props}>
			{label}
		</Inner>
	);
};
...
const Inner = styled.button<ButtonProps>`
	...
	background: ${({ styleConcept }) =>
		styleConcept === 'except' ? '#EEF1F1' : `var(${'--primary'})`};
	color: ${({ styleConcept }) =>
		styleConcept === 'except' ? '#7C8986' : `var(${'--white'})`};
	...
	:active {
		background: ${({ styleConcept }) =>
			styleConcept === 'except' ? '#D2D2D2' : `var(${'--primary'})`};
		color: ${({ styleConcept }) =>
			styleConcept === 'except' ? '#7C8986' : `var(${'--white'})`};
	}
